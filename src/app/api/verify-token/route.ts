...
export async function POST(request: NextRequest) {
  try {
    ...
    // Decode the ID token
    const tokenPayload = await decodeIdToken(idToken);
    ...
    // Check groups from token
    let groupResponse: GroupCheckResponse = checkUserGroups(tokenPayload);

    // If no groups in token and access token provided, fetch from Graph API
    if (groupResponse.groups.length === 0 && accessToken) {
      const fetchedGroups = await fetchUserGroups(accessToken);
      if (fetchedGroups.length > 0) {
        const adminGroupId = process.env.ADMIN_GROUP_ID || "";
        const viewGroupId = process.env.VIEW_GROUP_ID || "";
